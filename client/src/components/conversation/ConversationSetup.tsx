...
export function ConversationSetup({ scenario, onStart, isLoading }: { scenario: Scenario; onStart: (ctx: UserContext) => void; isLoading: boolean }) {
  ...
  return (
    <div className="min-h-full bg-neutral-50 flex items-center justify-center p-8">
      <div className="bg-white rounded-3xl shadow-sm border border-neutral-100 w-full max-w-2xl overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-primary-900 to-primary-600 p-8 rounded-t-3xl text-white">
          <span className="inline-flex bg-primary-800 text-primary-200 text-xs px-3 py-1 rounded-full mb-3">
            {scenario.topic.replace('_', ' ').toUpperCase()}
          </span>
          <h2 className="text-2xl font-bold mb-1">{scenario.name}</h2>
          <p className="text-primary-200 text-sm">{scenario.description}</p>
        </div>
        ...
        <div className="p-8 flex flex-col gap-6">
          ...
          {scenario.topic === 'job_interview' && (
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Role</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Company</label>
          ...
          {scenario.topic === 'daily_life' && (
            <div>
              <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Details (Optional)</label>
          ...
          {scenario.topic === 'college' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Subject</label>
                <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="e.g. Data Structures" className={inputClass} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">College</label>
          ...
          {/* Note */}
          <div className="bg-neutral-50 rounded-xl p-4 text-sm text-neutral-500 flex items-start gap-2 mt-0">
          ...
          <button onClick={handleStart} disabled={!canStart || isLoading}
            className="w-full bg-primary-600 hover:bg-primary-700 text-white py-3.5 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer active:scale-[0.98] mt-0">
